...
Product *load_products(const char *path) {
    FILE *fp = fopen("C:\\Apache24\\data\\snacks.txt", "r");
...
void save_products(const char *path, Product *list) {
    FILE *fp = fopen("C:\\Apache24\\data\\snacks.txt", "w");
...
void print_html(Product *list) {
    ...
    puts("<html><head><title>Snacks</title>"
         "<meta charset='UTF-8'>"
         "<style>"
    ...
    for (Product *p = list; p; p = p->next) {
        ...
        if (p->stock > 0)
            printf("<form method='GET' action='/cgi-bin/snacks.cgi'>"
...
/* ---------- main ---------- */
int main(void) {
    const char *file = "C:\\Apache24\\data\\snacks.txt";
