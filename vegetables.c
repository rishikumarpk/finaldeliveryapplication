...
Product *load_products(const char *path) {
    FILE *fp = fopen("C:\\Apache24\\data\\vegetables.txt", "r");
...
void save_products(const char *path, Product *list) {
    FILE *fp = fopen("C:\\Apache24\\data\\vegetables.txt", "w");
...
void print_html(Product *list) {
    ...
    puts("<html><head><title>Vegetables</title>"
         "<meta charset='UTF-8'>"
         "<style>"
         ...
         "form{margin-top:8px;}"
         "</style>"
         "</head><body><h2>🥕 Vegetables Store</h2>");
    ...
    for (Product *p = list; p; p = p->next) {
        ...
        if (p->stock > 0)
            printf("<form method='GET' action='/cgi-bin/vegetables.cgi'>"
...
/* ---------- main ---------- */
int main(void) {
    const char *file = "C:\\Apache24\\data\\vegetables.txt";
